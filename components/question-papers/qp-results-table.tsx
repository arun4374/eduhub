// components/qp-results-table.tsx  (Server Component — no client JS needed for the table itself)
    ...
    <div className="border border-[#E5E7EB] dark:border-[#2A2A2A] rounded-xl overflow-hidden bg-white dark:bg-[#1A1A1A] shadow-sm">
      ...
      <div className="hidden md:block">
        <Table id="query-qp-results-table">
          ...
          <TableBody>
            {documents.map((qp) => (
              <TableRow id={`row-qp-item-${qp._id}`} key={qp._id} className="hover:bg-indigo-50/5 dark:hover:bg-[#1E1E1E] transition-colors">
                ...
                <TableCell className="text-right">
                  <a id={`query-download-link-${qp._id}`} href={qp.file_url} target="_blank" rel="noopener noreferrer" className="inline-flex">
                    <Button variant="outline" size="sm" className="h-8 text-xs flex items-center gap-1 hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 shadow-none border-[#E5E7EB] dark:border-[#2A2A2A]">
        ...
        {documents.map((qp) => (
          <div id={`query-card-item-${qp._id}`} key={qp._id} className="p-4 flex flex-col justify-between gap-4 bg-white dark:bg-[#1A1A1A]">
            ...
            <a id={`query-download-link-mob-${qp._id}`} href={qp.file_url} target="_blank" rel="noopener noreferrer" className="block w-full">
              <Button variant="outline" className="w-full text-xs flex items-center justify-center gap-1.5 h-9">
